...
cleanup() {
    ...
    sed -i 's|^description=.*|description=「未运行😵」 基于 inotify 事件驱动与 mv 原子操作构建的文件重定向模块。|' "$MODDIR/module.prop"
...
# ─────────────────────────────────────────────
# MediaStore 通知
# 扫新路径写入完整元数据 → 等待索引完成 → 删两种路径形式的旧记录
...
_cascade_rmdir() {
    ...
    while [ "$i" -lt "$depth" ] && [ -n "$p" ]; do
        case "$p" in
            ...
            /sdcard/Documents|/storage/emulated/0/Documents|\
            /sdcard/Android|/storage/emulated/0/Android)
                log_msg "WARN" "级联清理空文件夹到达受保护目录\"$p\"，已停止清理"
                break ;;
        esac
    ...
    if [ "$dst_dir" = "__delete__" ]; then
        if rm "$src_file" 2>/dev/null; then
            ...
            content delete --uri content://media/external/images/media \
                --where "_data='${_e}'" >/dev/null 2>&1 || true
            # 级联清理空父目录
            if [ "${rule_depth:-0}" -gt 0 ] 2>/dev/null; then
...
monitor_inotifywait() {
    local parsed_rules="$1" inotifywait_bin="$2" is_first_run="${3:-0}" no_dir_warned="${4:-0}"
    ...
    [ -n "$pending_dirs" ] && [ "$is_first_run" = "1" ] && \
        log_msg "INFO" "解析规则：${_RULE_SUMMARY}；看门狗等待中：$(printf '%s\n' "$pending_dirs" | tr '\n' ' ')"

    rm -f "$EVT_FIFO"
    mkfifo "$EVT_FIFO" 2>/dev/null || { log_msg "ERROR" "无法创建 FIFO"; return 1; }

    if [ -z "$valid_dirs" ]; then
        [ "$no_dir_warned" = "0" ] && log_msg "WARN" "无可监控目录，等待看门狗..."
...
main() {
    ...
    if [ -f "$MODDIR/.reload_flag" ]; then
        ...
        log_msg "INFO" "热重启中 PID=$$"
    else
        log_msg "INFO" "启动中 PID=$$"
    fi
    sed -i 's|^description=.*|description=「启动中😇」 基于 inotify 事件驱动与 mv 原子操作构建的文件重定向模块。|' "$MODDIR/module.prop"
    ...
    _RULE_SUMMARY="${valid_count} 条有效，${invalid_count} 条无效"
    ...
    [ "$valid_count" -eq 0 ] && log_msg "WARN" "暂无有效规则"
    ...
    sed -i 's|^description=.*|description=「运行中😋」 基于 inotify 事件驱动与 mv 原子操作构建的文件重定向模块。|' "$MODDIR/module.prop"
    local _first_run=1
    local _rules_mtime="" all_parsed="" _loop_err=0
    local _no_dir_warned=0
    while true; do
        # 检测 KSU 是否已禁用本模块
        ...
        if [ "$cur_rules_mtime" != "$_rules_mtime" ]; then
            ...
            _no_dir_warned=0
            local _vc _ic
            _vc=$(printf '%s\n' "$all_parsed" | grep -c '^VALID|' 2>/dev/null || echo 0)
            _ic=$(printf '%s\n' "$all_parsed" | grep -c '^INVALID|' 2>/dev/null || echo 0)
            [ -z "$(printf '%s\n' "$all_parsed" | grep '^VALID|')" ]   && _vc=0
            [ -z "$(printf '%s\n' "$all_parsed" | grep '^INVALID|')" ] && _ic=0
            _RULE_SUMMARY="${_vc} 条有效，${_ic} 条无效"
        fi
        monitor_inotifywait "$all_parsed" "$inotifywait_bin" "$_first_run" "$_no_dir_warned" || true
        # 本轮结束后检查是否仍无可用目录，决定下轮是否继续抑制警告
        if [ -n "$(printf '%s\n' "$all_parsed" | grep '^VALID|' | cut -d'|' -f2 | while IFS= read -r d; do [ -d "$d" ] && printf 'y' && break; done)" ]; then
            _no_dir_warned=0
        else
            _no_dir_warned=1
        fi
